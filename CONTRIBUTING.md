...
Thank you for considering making contributions to Pstake and related
repositories!
...
1. Either [open](https://github.com/persistenceOne/pstake-native/issues/new/choose) or
   [find](https://github.com/persistenceOne/pstake-native/issues) an issue you'd like to help with
...
3. If you would like to contribute:
   ...
   4. Follow standard Github best practices: fork the repo, branch from the
      ...
      - For core developers working within the Pstake repo, to ensure a clear
...
- Looking for a good place to start contributing? How about checking out some
  [good first issues](https://github.com/persistenceOne/pstake-native/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22)
...
The following PR structuring checklist can be used when submitting changes to the Pstake repository for review:
...
If you open a PR in Pstake, it is mandatory to update the relevant documentation in /docs.

- If your changes relate specifically to the pstake application, please modify the docs/ folder.

## Forking

Please note that Go requires code to live under absolute paths, which complicates forking.
While my fork lives at `https://github.com/<username>/pstake-native`,
the code should never exist at  `$GOPATH/src/github.com/<username>/pstake-native`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,
`$GOPATH/src/github.com/persistenceOne/pstake-native`, and do all the work there.

For instance, to create a fork and work on a branch of it, I would:

- Create the fork on github, using the fork button.
- Go to the original repo checked out locally (i.e. `$GOPATH/src/github.com/persistenceOne/pstake-native`)
- `git remote rename origin upstream`
- `git remote add origin git@github.com:<username>/pstake-native.git`

Now `origin` refers to my fork and `upstream` refers to the Pstake version.
So I can `git push -u origin main` to update my fork, and make pull requests to Pstake from there.
...
When dependencies in Pstake-native's `go.mod` are changed, it is generally accepted practice
...
Pstake-native utilizes [semantic versioning](https://semver.org/).
...
- create a development branch either on `https://github.com/persistenceOne/pstake-native`, or your fork (using `git remote add origin`)
