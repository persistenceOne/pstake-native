...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Redeem {} => {
            ...
            let burn_msg = CosmosMsg::Bank(BankMsg::Burn { amount: info.funds });
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Config { stk_asset } => to_json_binary(&CONFIG.load(deps.storage, &stk_asset)?),
        QueryMsg::Configs {} => {
...
mod tests {
    ...
    fn test_contract() {
        ...
        let mut app = App::new(|router, _, storage| {
            ...
            router
                .bank
                .init_balance(storage, &admin_addr, coins1)
                .unwrap()
        });

        let redeem_contract = Box::new(ContractWrapper::new_with_empty(
            contract::execute,
        ...
        let res = app
            .execute_contract(
                admin_addr.clone(),
                contract_i.clone(),
                &contract::ExecuteMsg::UpdateConfig {
                    base_asset: "ibc/uatom".to_string(),
                    stk_asset: "stk/uatom".to_string(),
                    exchange_rate: ex,
                },
                &vec![coins(1000_000_00, "ibc/uatom")[0].clone()],
            )
            .unwrap();
        println!("{:?}", res);

        let q1: Vec<(Addr, CfgData)> = app
            .wrap()
            .query_wasm_smart(&contract_i.clone(), { &QueryMsg::Configs {} })
            .unwrap();
        println!("{:?}", q1);
        ...
        let res1 = app
            .execute_contract(
                admin_addr.clone(),
                contract_i.clone(),
                &contract::ExecuteMsg::Redeem {},
                &vec![coins(1000_000, "stk/uatom")[0].clone()],
            )
            .unwrap();
        println!("{:?}", res1);
    }
