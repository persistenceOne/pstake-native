...
// DefaultParams returns the default lspersistence module parameters.
...
// validateWhitelistedValidators validates lspersistence validator and total weight.
func validateWhitelistedValidators(i interface{}) error {
	...
	for _, wv := range wvs {
		...
		if wv.TargetWeight.IsNil() {
			return fmt.Errorf("lspersistence validator target weight must not be nil")
		}

		if !wv.TargetWeight.IsPositive() {
			return fmt.Errorf("lspersistence validator target weight must be positive: %s", wv.TargetWeight)
		}

		if _, ok := valsMap[wv.ValidatorAddress]; ok {
			return fmt.Errorf("lspersistence validator cannot be duplicated: %s", wv.ValidatorAddress)
		}
