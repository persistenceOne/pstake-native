	...
	for _, lv := range liquidValidators {
		if !k.IsActiveLiquidValidator(ctx, lv, whitelistedValsMap) {
			...
			if delShares.IsPositive() {
				cachedCtx, writeCache := ctx.CacheContext()
				completionTime, returnAmount, _, err := k.LiquidUnbond(cachedCtx, types.LiquidStakingProxyAcc, types.LiquidStakingProxyAcc, lv.GetOperator(), delShares, false, k.stakingKeeper.UnbondingTime(ctx))
				if err != nil {
