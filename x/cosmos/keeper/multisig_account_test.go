...
func multisig(OrcastratorAddresses []string, k keeper.Keeper, accountKeeper authKeeper.AccountKeeper, t *testing.T, ctx sdk.Context, accountNumber uint64, threshold int64) *authTypes.BaseAccount {
	...
	for _, orcastratorAddress := range OrcastratorAddresses {
		...
		account := accountKeeper.GetAccount(ctx, orchestratorAccAddress)
...
func TestKeeper_SetAccountState(t *testing.T) {
	...
	appAuthKeeper := app.AccountKeeper
	...
	appAuthKeeper.SetAccount(ctx, acc)
	baseAccount := multisig([]string{orcastratorAddress}, cosmosKeeper, appAuthKeeper, t, ctx, 0, 1)
