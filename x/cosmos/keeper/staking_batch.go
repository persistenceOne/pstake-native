...
// addToStakingEpoch Adds the given amount to the current "stake" epoch
...
// getAmountFromStakingEpoch Gets the amount from the "stake" epoch store with the given epoch number
func (k Keeper) getAmountFromStakingEpoch(ctx sdk.Context, epochNumber int64) (amount sdk.Coin) {
	...
	if !stakingEpochStore.Has(cosmosTypes.Int64Bytes(epochNumber)) {
		return sdk.NewInt64Coin(cosmosTypes.DefaultStakingDenom, 0)
	}
	k.cdc.MustUnmarshal(stakingEpochStore.Get(cosmosTypes.Int64Bytes(epochNumber)), &amount)
	return amount
}

// deleteFromStakingEpoch Removes the entry from the "stake" epoch store
